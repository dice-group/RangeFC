...
def select_model(args) -> Tuple[pl.LightningModule, AnyStr]:
    ...
    #     other fact checking models (#tobeupdated later)
        ...
        model = RangeMLPModel(
            ...
            w_ce=getattr(args, "w_ce", 0.08),
            w_iou=getattr(args, "w_iou", 0.20),
            w_consistency=getattr(args, "w_consistency", 0.01),
            w_end_boost=getattr(args, "w_end_boost", 0.02),
            w_order=getattr(args, "w_order", 0.01),
            w_align=getattr(args, "w_align", 0.01),
            w_smooth=getattr(args, "w_smooth", 0.010),
            w_ridge=getattr(args, "w_ridge", 0.0003),
            w_reg=getattr(args, "w_reg", 1.0),
            pred_head=getattr(args, "pred_head", "calendar"),
